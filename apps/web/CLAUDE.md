...
- Raw WebGL2 only — no Three.js/Pixi. Assume WebGL2 features are available.
- **Isolate GL behind composables/modules** (e.g. `src/composables/` or a dedicated `src/gl/` module). Components should not touch the raw context directly.
- **Own the lifecycle explicitly.** Create context, programs, and buffers deliberately; dispose of every GL resource and cancel the render loop on `onUnmounted`. Route changes must not leak contexts.
